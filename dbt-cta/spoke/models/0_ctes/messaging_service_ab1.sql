...
-- depends_on: {{ source('public', '_airbyte_raw_messaging_service') }}
select
    ...
    {{ json_extract_scalar('_airbyte_data', ['service_type'], ['service_type']) }} as service_type,
    {{ json_extract_scalar('_airbyte_data', ['organization_id'], ['organization_id']) }} as organization_id,
    {{ json_extract_scalar('_airbyte_data', ['encrypted_auth_token'], ['encrypted_auth_token']) }} as encrypted_auth_token,
    {{ json_extract_scalar('_airbyte_data', ['messaging_service_sid'], ['messaging_service_sid']) }} as messaging_service_sid,
    _airbyte_ab_id,
    _airbyte_emitted_at,
    {{ current_timestamp() }} as _airbyte_normalized_at
from {{ source('public', '_airbyte_raw_messaging_service') }} as table_alias
-- messaging_service
where 1 = 1
