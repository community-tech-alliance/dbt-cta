...
-- depends_on: {{ source('cta', '_raw_redshift_people_shorttext_conversations') }}
...
select
    ...
    {{ dbt_utils.surrogate_key([
        ...
        'question_text',
        'response'
    ]) }} as _redshift_people_shorttext_conversations_hashid,
    {{ current_timestamp() }} as _cta_loaded_at
from {{ source('cta', '_raw_redshift_people_shorttext_conversations') }}
where 1 = 1
