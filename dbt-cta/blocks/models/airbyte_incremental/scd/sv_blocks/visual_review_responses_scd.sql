{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='visual_review_responses'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} final_table where final_table._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('visual_review_responses')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('visual_review_responses')) }}
                    ...
                    {% endif %}
                    ","drop view _airbyte_sv_blocks.visual_review_responses_stg"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('visual_review_responses_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('visual_review_responses_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('visual_review_responses_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('visual_review_responses_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('visual_review_responses_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
