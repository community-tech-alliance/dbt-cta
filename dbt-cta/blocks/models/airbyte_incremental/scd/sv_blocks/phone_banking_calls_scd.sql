{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='phone_banking_calls'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} final_table where final_table._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('phone_banking_calls')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('phone_banking_calls')) }}
                    ...
                    {% endif %}
                    ","drop view _airbyte_sv_blocks.phone_banking_calls_stg"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('phone_banking_calls_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('phone_banking_calls_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('phone_banking_calls_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('phone_banking_calls_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('phone_banking_calls_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
