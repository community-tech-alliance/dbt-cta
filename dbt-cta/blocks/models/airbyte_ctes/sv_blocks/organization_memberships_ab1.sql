...
-- depends_on: {{ source('sv_blocks', '_airbyte_raw_organization_memberships') }}
select
    {{ json_extract_scalar('_airbyte_data', ['member_id'], ['member_id']) }} as member_id,
...
from {{ source('sv_blocks', '_airbyte_raw_organization_memberships') }} as table_alias
