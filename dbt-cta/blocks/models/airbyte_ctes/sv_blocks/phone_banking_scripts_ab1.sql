...
-- depends_on: {{ source('sv_blocks', '_airbyte_raw_phone_banking_scripts') }}
select
    ...
    {{ json_extract_scalar('_airbyte_data', ['cause_statement'], ['cause_statement']) }} as cause_statement,
    {{ json_extract_scalar('_airbyte_data', ['created_by_user_id'], ['created_by_user_id']) }} as created_by_user_id,
    _airbyte_ab_id,
    _airbyte_emitted_at,
    {{ current_timestamp() }} as _airbyte_normalized_at
from {{ source('sv_blocks', '_airbyte_raw_phone_banking_scripts') }} as table_alias
