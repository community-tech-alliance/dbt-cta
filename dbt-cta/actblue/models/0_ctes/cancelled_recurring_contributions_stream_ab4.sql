-- ensures the base model contains only one row per Receipt_ID
...
-- Receipt_ID in the base model would fail with "UPDATE/MERGE must match at
...
select * except (rownum) from
    (
        select
            *,
            row_number() over (partition by Receipt_ID order by _airbyte_extracted_at desc) as rownum
