...
with stats as (
  ...
  *,
  ...
  --------------------------------
  case when impressions>0 then inline_link_clicks/impressions else null end as ctr_calculated,
  case when impressions>0 then landing_page_views/impressions else null end as page_view_rate_calculated,
  case when impressions>0 then video_p100_watched/impressions else null end as video_completion_rate_calculated

	from {{ ref('cta_ads_insights_platform_and_device') }}
),
ads as (
	select
     a.id as ad_id
    ...
    ,c.utm_term
    ,c.utm_content
	from {{ ref('cta_ads') }} as a
	left join {{ ref('cta_ad_creatives') }} as c
...
select
  ...
  ,stats.date_stop
  ,stats.ad_id
  ,stats.publisher_platform
  ,stats.platform_position
  ,stats.impression_device
  ,join2.adset_id
  ,join2.campaign_id
  ,join2.campaign_name
  ...
  ,stats.video_played
  ,stats.video_continuous_2_sec_watched_actions
