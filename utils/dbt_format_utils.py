...
def main():
    parser = argparse.ArgumentParser()
    sub_parsers = parser.add_subparsers(title="action", description="Choose which action to run", dest="action")
    ...
    if args.action == "renameBaseTables":
        ...
        if not args.outputPath:
            raise Exception("Make sure you specify the --outputPath argument")
        # Run selected action
        download_cacher_script(args.cacherSnippetGUID, args.cacherApiKey, args.cacherApiToken, args.outputPath)
        ...
        raise Exception("Please enter a supported action -> renameBaseTables, createMatViews, getCacherScript, restructAirbyteDbt")
...
def restructure_airbyte_dbt(airbyte_workspace_path, dbt_cta_path):
    ...
    for airbyte_dir, cta_dir in airbyte_cta_dir_mapping.items():
        ...
        files_to_move = glob(f"{airbyte_workspace_path}/**/{airbyte_dir}/**/*.sql", recursive=True)

        if files_to_move:  # Only run if there were any files found
            # Make sure target path is created
            dbt_cta_path = dbt_cta_path if dbt_cta_path[-1] != "/" else dbt_cta_path[:-1]
    ...
    sources_file = glob(f"{airbyte_workspace_path}/**/models/**/sources.yml", recursive=True)
    if sources_file:
        move_source_file = move(
            src=sources_file[0],
            dst=f"{dbt_cta_path}/models/sources.yml"
        )
...
def download_cacher_script(script_guid, api_key, api_token, output_path):
    ...
    # Find target snippet
    target_script = next(snippet for snippet in cta_snippets if snippet.get("guid") == script_guid)
    script_content = target_script.get("files")[0]["content"]

    with open(output_path, "w") as output_file:
        output_file.write(script_content)

    print(f"Cacher Snippet {target_script.get('title', script_guid)} downloaded to {output_path}")
    
# Function to append "_base" to all files in a directory
def add_base_to_filenames(base_tables_path):
    base_tables = [
        f for f in os.listdir(base_tables_path) if os.path.isfile(os.path.join(base_tables_path, f))
    ]
    base_tables.sort()
...
def create_matview_dbt_files_from_base(base_tables_path, output_path):
    ...
    base_tables_path = base_tables_path if base_tables_path[-1] != "/" else base_tables_path[:-1]
