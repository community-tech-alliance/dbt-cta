...
def generate_file_contents_cte(select_statement, table_id, cte_number: int):
    """
...
def generate_file_contents_base(data_fields_and_types, table_id, sync_mode, unique_key):
    """
...
def generate_file_contents_matview(data_fields_and_types, table_id):
    field_names_list = list(data_fields_and_types.keys())
    field_names_formatted = (",\n    ").join(field_names_list)
...
def create_sources_yaml(tables_list, models_dir):
    ...
    source_tables_list = [
        {"name": f"_airbyte_raw_{table}"} for table in tables_list
...
def write_dbt_file_cte1(table_id, data_fields_and_types, models_dir):
    ...
    cte1_file_contents = generate_file_contents_cte(
        select_statement=cte1_select_statement, table_id=table_id, cte_number=1
    )
...
def write_dbt_file_cte2(table_id, data_fields_and_types, models_dir):
    cte2_select_statement = generate_cte2_select_statement(
        data_fields_and_types=data_fields_and_types
    )

    cte2_file_contents = generate_file_contents_cte(
        select_statement=cte2_select_statement, table_id=table_id, cte_number=2
    )
...
def write_dbt_file_cte3(table_id, data_fields_and_types, models_dir):
    cte3_select_statement = generate_cte3_select_statement(
        table_id=table_id, data_fields_and_types=data_fields_and_types
    )

    cte3_file_contents = generate_file_contents_cte(
        select_statement=cte3_select_statement, table_id=table_id, cte_number=3
    )
...
def write_dbt_file_base(
    table_id, data_fields_and_types, models_dir, sync_mode, unique_key
):
...
def write_dbt_file_matview(table_id, data_fields_and_types, models_dir):
    matview_file_contents = generate_file_contents_matview(
        data_fields_and_types=data_fields_and_types, table_id=table_id
    )
...
def write_dbt_models_for_table(
    table_id, data_fields_and_types, models_dir, sync_mode, unique_key
):
    """
