...
from dbt_file_contents import get_base_sql, get_base_config, get_matview_sql
...
def main():
    ...
    parser.add_argument("--project_id", "-p", help="Google project id.")
    parser.add_argument("--dataset_id", "-d", help="Dataset id.")
    parser.add_argument(
        "--output_path",
        "-o",
    ...
    with tempfile.TemporaryDirectory() as temp_dir_TODO:
        ...
        models_path = os.path.join(temp_dir, sync_dir, models_dir)
        ...
        for table in tables:
            ...
            partition_datetime_field = spec_json_dict[table_id_raw].get(
                "partition_datetime_field", "_cta_sync_datetime_utc"
            )
            ...
            models_subdir_path = os.path.join(models_path, models_subdir)
            ...
            concat_fields = ",\n                                        ".join(
                f"`{field.name}`" for field in table_schema
            )
            ...
            # Create the subdirectory if it doesn't already exist
            matviews_subdir = "2_partner_matviews"
            matviews_subdir_path = os.path.join(models_path, matviews_subdir)
            os.makedirs(matviews_subdir_path, exist_ok=True)

            matview_fields = ",\n    ".join(f"{field.name}" for field in fields_revised)
            ...
            matview_file_path = os.path.join(
                matviews_subdir_path, f"{table_id_raw}.sql"
            )
