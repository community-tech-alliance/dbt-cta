...
-- depends_on: {{ ref('charges_payment_method_details') }}
select
    _airbyte_payment_method_details_hashid,
    {{ json_extract_scalar('card', ['id'], ['id']) }} as id,
    {{ json_extract('table_alias', 'card', ['eps'], ['eps']) }} as eps,
    {{ json_extract('table_alias', 'card', ['p24'], ['p24']) }} as p24,
    ...
    {{ json_extract('table_alias', 'card', ['ideal'], ['ideal']) }} as ideal,
    {{ json_extract_scalar('card', ['last4'], ['last4']) }} as last4,
    {{ json_extract('table_alias', 'card', ['checks'], ['checks']) }} as checks,
    {{ json_extract('table_alias', 'card', ['klarna'], ['klarna']) }} as klarna,
    {{ json_extract_scalar('card', ['object'], ['object']) }} as object,
    {{ json_extract('table_alias', 'card', ['sofort'], ['sofort']) }} as sofort,
    {{ json_extract('table_alias', 'card', ['wallet'], ['wallet']) }} as wallet,
    {{ json_extract('table_alias', 'card', ['wechat'], ['wechat']) }} as wechat,
    {{ json_extract_scalar('card', ['country'], ['country']) }} as country,
    {{ json_extract_scalar('card', ['funding'], ['funding']) }} as funding,
    {{ json_extract('table_alias', 'card', ['giropay'], ['giropay']) }} as giropay,
    {{ json_extract_scalar('card', ['network'], ['network']) }} as network,
    ...
    {{ json_extract('table_alias', 'card', ['multibanco'], ['multibanco']) }} as multibanco,
    {{ json_extract('table_alias', 'card', ['sepa_debit'], ['sepa_debit']) }} as sepa_debit,
    ...
    {{ json_extract('table_alias', 'card', ['card_present'], ['card_present']) }} as card_present,
    {{ json_extract('table_alias', 'card', ['installments'], ['installments']) }} as installments,
    ...
    {{ json_extract('table_alias', 'card', ['stripe_account'], ['stripe_account']) }} as stripe_account,
    {{ json_extract('table_alias', 'card', ['three_d_secure'], ['three_d_secure']) }} as three_d_secure,
